...
export const createFile = async (
  ...
  try {
    const globalPath = await userService.getGlobalRootDir(pool, userId);
    const path = pathPkg.join(globalPath, file.fields.fileParent.value);
    ...
    const directory = await dirService.createPath(pool, path);
 ...
 * @param {String} storageId
 */
export const getFileByPath = async (pool, filename, pathStr, storageId) => {
  const dir = await dirService.getDirByPath(pool, pathStr);
...
export const moveFile = async (pool, payload) => {
  const { id, updatedPath: newPath, storageId, userId } = payload;
  const globalPath = await userService.getGlobalRootDir(pool, userId);
  const updatedPath = pathPkg.join(globalPath, newPath);
  ...
  const exist = await getFileByPath(pool, filename, updatedPath, storageId);

  if (exist.code === 200) {
    return { code: 400, msg: 'File with same name exists in ' + updatedPath };
  }

  const directory = await dirService.createPath(pool, updatedPath);
...
export const copyFile = async (pool, payload) => {
  ...
  const globalPath = await userService.getGlobalRootDir(pool, userId);
  const updatedPath = pathPkg.join(globalPath, path);
  ...
  const exists = await getFileByPath(pool, filename, updatedPath, storageId);
  if (exists) {
