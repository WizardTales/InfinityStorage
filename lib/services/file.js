...
export const getFiles = async (
  ...
  try {
    ...
    const query = SQL`SELECT max(v.version) as version, f.id, f.filename, f.size, f.mime, f.state, f.data, l."expireAt" as "lockExpiresAt"
      FROM "file" f
      INNER JOIN "filePermission" fp ON fp."fileId" = f.id
      INNER JOIN "version" v ON f.id = v."fileId"
...
export const downloadFile = async (
  ...
  try {
    ...
    const exists = await minioService.exists(minioClient, id);
