...
export async function lock (pool, id, storageId) {
  const chkQuery = SQL`SELECT l.id, l."expireAt" FROM file f
  ...
  await pool.retry(async (client) => {
    ...
    if (lockId) {
      ...
      query = SQL`UPDATE "lock"
        SET "expireAt" = '${raw(`NOW() + '${lockDuration} seconds'::INTERVAL`)}'
        WHERE id = ${lockId}`;
    } else {
...
export async function unlock (pool, id, storageId) {
  const chkQuery = SQL`SELECT f.id, l.id as "lockId", l."expireAt" FROM file f
    LEFT JOIN lock l ON l."fileId" = f.id
    WHERE f.id = ${id} AND f."storageId" = ${storageId}`;
  const {
    rows: [{ lockId, expireAt }]
  } = await pool.query(chkQuery);
  ...
  await pool.retry(async (client) => {
    const query = SQL`UPDATE lock SET "expireAt" = 'NOW()' WHERE id = ${lockId}`;
