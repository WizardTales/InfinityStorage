...
async function existsNameInPath (pool, name, path, storageId, userId) {
  const q = SQL`SELECT f.id FROM file f
  INNER JOIN directory d ON f."directoryId" = d.id
  INNER JOIN "user" u ON d."parentId" = u."globalDirId"
  WHERE f.filename = ${name} AND d.path = ${path}
  AND u.id = ${userId} AND f."storageId" = ${storageId}`;
