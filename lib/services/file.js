...
export const getFiles = async (pool, storageId, page = 0, length = 20) => {
  try {
    const query = SQL`SELECT f.id, f.filename, f.size, f.mime, f.state, f.data, l."expiryAt" as "lockExpiresAt"
      FROM "file" f
...
export const deleteFile = async (pool, minioClient, id, storageId) => {
  const query = SQL`SELECT f."extFilename", l."expiryAt" FROM "file" f
    LEFT JOIN "lock" l ON f.id = l."fileId"
    WHERE f.id = ${id}`;
  ...
  const { extFilename, expiryAt } = rows[0];

  if (expiryAt && dayjs().isBefore(expiryAt)) {
...
export const moveFile = async (pool, payload) => {
  ...
  const { expiryAt, filename } = file.data;
...
export async function lock (pool, id, storageId) {
  ...
  const chkQuery = SQL`SELECT l.id, l."expiryAt" FROM file f
  ...
  await pool.retry(async (client) => {
    ...
    if (lockId) {
      ...
      query = SQL`UPDATE "lock"
        SET "expiryAt" = '${raw(`NOW() + '${lockDuration} seconds'::INTERVAL`)}'
        WHERE id = ${lockId}`;
    } else {
      query = SQL`INSERT INTO "lock" ("fileId", "expiryAt")
        VALUES (${id}, '${raw(
        `NOW() + '${lockDuration} seconds'::INTERVAL`
      )}')`;
    }
...
export async function unlock (pool, id, storageId) {
  const chkQuery = SQL`SELECT f.id, l.id as "lockId", l."expiryAt" FROM file f
    LEFT JOIN lock l ON l."fileId" = f.id
    WHERE f.id = ${id} AND f."storageId" = ${storageId}`;
  const {
    rows: [{ lockId, expiryAt }]
  } = await pool.query(chkQuery);

  if (!expiryAt || dayjs().isAfter(expiryAt)) {
    return { code: 204, msg: 'File is not locked' };
  }

  await pool.retry(async (client) => {
    const query = SQL`DELETE FROM lock WHERE id = ${lockId}`;
