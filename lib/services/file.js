...
export const createFile = async (pool, minioClient, file, storageId) => {
  try {
    ...
    if (existsFile) {
      return { code: 400, msg: 'File already exists in the location' };
    }
...
export const getFiles = async (pool, storageId, page = 0, length = 20) => {
  try {
    const query = SQL`SELECT id, filename, size, mime, state, data, locked
...
export const getFileById = async (pool, id, storageId) => {
  try {
    const query = SQL`SELECT id, filename, size, mime, state, data, path, locked
...
export const getFileByNameAndPath = async (
  ...
  const q = SQL`SELECT id, filename, size, mime, state, data, path, locked FROM "file"
    WHERE filename = ${filename} AND path = ${path} AND "storageId" = ${storageId}`;
...
export const downloadFile = async (pool, minioClient, id, storageId) => {
  try {
    const query = SQL`SELECT id, filename, size, mime, state, data, "extFilename", locked
      FROM "file" WHERE "id" = ${id} AND "storageId" = ${storageId}`;
...
export const deleteFile = async (pool, minioClient, id, storageId) => {
  const query = SQL`SELECT "extFilename", locked FROM "file" WHERE id = ${id}`;
  ...
  const { extFilename, locked } = rows[0];

  if (locked) {
    return { code: 400, msg: 'File locked' };
  }
