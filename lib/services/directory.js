...
export async function create (pool, name, parentId = null) {
  ...
  const q = SQL`INSERT INTO directory ("name", "parentId")
    VALUES (${name}, ${parentId}) RETURNING *`;
...
export async function get (pool, name, parentId = null) {
  ...
  const q = SQL`SELECT id, name, "parentId", "createdAt", "updatedAt"
...
export async function getDirByPath (pool, pathStr) {
  try {
    ...
    return await dirArr.reduce(async (parent, dirname) => {
      return get(pool, dirname, (await parent).id);
...
export async function move (pool, id, newParentId = null) {
  const q = SQL`UPDATE directory SET "parentId" = ${newParentId}
