...
export default async function (request, reply) {
    ...
    const _authDriver = {
      getSalt: async (username) => {
        ...
        const { rows } = await pool.query(
          'SELECT "salt", "data" FROM "user" WHERE "username" = $1',
          [username]
        );

        if (rows[0]) {
          rows[0].extend = rows[0].data.extend;
          return rows[0];
        } else return null;
      },
  ...
  if (request.cookies[config.cookies.cookie]) {
    request.session = await reply.decryptCookie(
      request.cookies[config.cookies.cookie]
    );
  }
}
