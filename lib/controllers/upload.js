...
export default {
  ...
  async getStorage (request, h) {
    ...
    const res = await request.seneca
      .actAsync('service:user,command:get,asset:files', {
        session,
        data: {
          storageId
        }
  ...
  async deleteFile (request, h) {
    ...
    let res = await request.seneca.actAsync('service:user,command:delete,asset:file', {
      session,
      data: { id }
    });
    ...
    res = await request.seneca.actAsync('service:user,command:delete,asset:file', {
      session,
  ...
  async upload (request, h) {
    ...
    const res = await request.seneca.actAsync('service:user,command:check,asset:storageReq', {
      session,
    ...
    const { msg: { file: fileRef } } = await request.seneca.actAsync('service:user,command:create,asset:file', {
      session,
      data: { filename: file.hapi.filename, mime, ip, storageId }
    });

    await new Promise((resolve, reject) => {
      s3.putObject(s3Config.bucket, `${storageId}/${filename}`, fileStream, (err) => {
        if (err) {
          console.log(err);
          return reject(err);
        }
  ...
  async download (request, h) {
    ...
    const res = await request.seneca.actAsync('service:user,command:get,asset:file', {
      session,
      data: { id, ip }
    });
    ...
    const file = await new Promise((resolve, reject) => {
      s3.presignedGetObject(s3Config.bucket, `${storageId}/${filename}`, 15, (err, url) => {
        if (err) {
          return reject(err);
        }
