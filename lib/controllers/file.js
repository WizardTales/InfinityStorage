...
export default {
  ...
  async createFile (request, reply) {
    try {
      ...
      const serverPart = data.fields.serverPart?.value || null;
      const userPart = data.fields.userPart?.value || null;
      let encryptionKey = null;

      if (serverPart && userPart) {
        encryptionKey = crypto.parse(serverPart, userPart);
      }
  ...
  async downloadFile (request, reply) {
    ...
    if (code === 200) {
      ...
      if (meta.encryptionKey) {
        headers['Key-Server-Part'] = meta.encryptionKey.serverPart;
        headers['Key-User-Part'] = meta.encryptionKey.userPart;
        headers['Access-Control-Expose-Headers'] =
          'Key-Server-Part, Key-User-Part';
      }
