...
export default {
  ...
  async createFile (request, reply) {
    ...
    reply.send(
      await service.createFile(pool, minioCient, data, userId, storageId)
    );
  },
  ...
  async getFileById (request, reply) {
    ...
    reply.send(await service.getFileById(pool, id, userId, storageId));
  },
  ...
  async lock (request, reply) {
    try {
      ...
      const { code, msg } = await service.lock(pool, id, storageId);
  ...
  async unlock (request, reply) {
    try {
      ...
      const { code, msg } = await service.unlock(pool, id, storageId);
